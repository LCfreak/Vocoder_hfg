...
# Performance flags
torch.backends.cudnn.benchmark = True
# Allow TF32 matmul (useful on Ampere/Turing for additional speed)
...
def train(h, args):
    ...
    # Keep user's batch size; do not alter hyperparameters automatically.
    train_loader = DataLoader(
        trainset, num_workers=max(1, int(h.num_workers)), shuffle=True,
        batch_size=h.batch_size, pin_memory=True, drop_last=True
    )
    ...
    # AMP setup
    use_amp = not args.disable_amp  # default: AMP enabled unless user passes --disable_amp
    if use_amp:
        scaler = torch.cuda.amp.GradScaler()
        print("✓ AMP (mixed precision) enabled")
    else:
        scaler = None
        print("✓ AMP disabled (using full precision)")
        ...
        for i, batch in enumerate(train_loader):
            ...
            if use_amp:
                with torch.cuda.amp.autocast():
                    y_g_hat = generator(x)
                    # mel spectrogram of generated (autocast is safe here)
                    y_g_hat_mel = mel_spectrogram(
                        y_g_hat.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate,
                        h.hop_size, h.win_size, h.fmin, h.fmax_for_loss
                    )
            ...
            if use_amp:
                with torch.cuda.amp.autocast():
                    # generator forward
                    # Re-generate (avoid using stale y_g_hat from D path in case autograd graph was freed)
                    y_g_hat = generator(x)
                    y_g_hat_mel = mel_spectrogram(
                        y_g_hat.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate,
                        h.hop_size, h.win_size, h.fmin, h.fmax_for_loss
            ...
            if steps % args.stdout_interval == 0:
                elapsed = time.time() - start_time
                # try to safely fetch scalars (they might not exist in FP32 path)
                try:
                    mel_val = loss_mel.item()
                except Exception:
                    mel_val = float('nan')
                try:
                    gen_val = loss_gen_all.item()
                except Exception:
                    gen_val = float('nan')
                try:
                    disc_val = loss_disc_all.item()
                except Exception:
                    disc_val = float('nan')

                print(f"Steps: {steps:7d} | Mel: {mel_val:.4f} | "
                      f"Gen: {gen_val:.4f} | Disc: {disc_val:.4f} | "
                      f"Time: {elapsed:.1f}s")

            if steps % args.summary_interval == 0:
                # prefer tensor values to be plain floats if possible
                try:
                    sw.add_scalar("training/gen_loss_total", float(loss_gen_all), steps)
                    sw.add_scalar("training/mel_spec_error", float(loss_mel), steps)
                    sw.add_scalar("training/disc_loss_total", float(loss_disc_all), steps)
                except Exception:
                    # If tensors are not defined (e.g. rare race), skip gracefully
                    pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_validation_file', required=True)
    parser.add_argument('--input_wavs_dir', required=True)
    # Allow user to disable AMP if they prefer
    parser.add_argument('--disable_amp', action='store_true', help='Disable AMP (mixed precision)')
